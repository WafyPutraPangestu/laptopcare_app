<div class="home-section-inner py-8">
    ...
    <div class="flex items-center justify-between mb-8">
        <div>
            ...
            <h1 class="home-section-title mb-1">Merek Laptop</h1>
            <p class="text-sm" style="color: var(--c-text-dim);">Kelola data merek dan spesifikasi laptop yang terdaftar
            </p>
        </div>
        <a href="{{ route('kepala.merek.create') }}" wire:navigate
    ...
    <div class="mb-5">
        <div class="relative" style="max-width: 360px;">
            ...
            <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari nama merek..."
            ...
            <thead>
                <tr style="background: var(--c-surface-2); border-bottom: 1px solid var(--c-border);">
                    <th class="px-5 py-3 text-left font-medium" style="color: var(--c-text-dim);">
                        <button wire:click="sortBy('nama_merek')"
                            class="flex items-center gap-1 hover:opacity-80 transition-opacity">
                            Nama Merek
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2"
                                style="color: {{ $sortField === 'nama_merek' ? 'var(--c-accent)' : 'var(--c-text-muted)' }}; transform: {{ $sortField === 'nama_merek' && $sortDirection === 'desc' ? 'rotate(180deg)' : 'none' }}">
                                <path d="M12 5l7 7-7 7M5 12h14" transform="rotate(-90 12 12)" />
                            </svg>
                        </button>
                    </th>
                    <th class="px-5 py-3 text-left font-medium" style="color: var(--c-text-dim);">
                        <button wire:click="sortBy('tahun_rilis')"
                            class="flex items-center gap-1 hover:opacity-80 transition-opacity">
                            Tahun Rilis
                            <svg width="10" height="10" viewBox="0 0 24 24" fill="none"
                                stroke="currentColor" stroke-width="2"
                                style="color: {{ $sortField === 'tahun_rilis' ? 'var(--c-accent)' : 'var(--c-text-muted)' }}; transform: {{ $sortField === 'tahun_rilis' && $sortDirection === 'desc' ? 'rotate(180deg)' : 'none' }}">
                                <path d="M12 5l7 7-7 7M5 12h14" transform="rotate(-90 12 12)" />
                            </svg>
                        </button>
                    </th>
                    <th class="px-5 py-3 text-left font-medium" style="color: var(--c-text-dim);">Usia Optimal</th>
                    <th class="px-5 py-3 text-left font-medium" style="color: var(--c-text-dim);">Jumlah Laptop</th>
                    <th class="px-5 py-3 text-left font-medium" style="color: var(--c-text-dim);">Spesifikasi</th>
                    ...
                    <tr class="border-b transition-colors hover:bg-opacity-50" style="border-color: var(--c-border);"
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center gap-3">
                                <div class="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0"
                                    style="background: var(--c-accent-dim); color: var(--c-accent); font-family: var(--font-mono); font-size: 0.625rem; font-weight: 700;">
                                    {{ strtoupper(substr($merek->nama_merek, 0, 2)) }}
                                </div>
                                <span class="font-semibold"
                                    style="color: var(--c-text);">{{ $merek->nama_merek }}</span>
                            </div>
                        </td>
                        <td class="px-5 py-4"
                            style="color: var(--c-text-dim); font-family: var(--font-mono); font-size: 0.8125rem;">
                            {{ $merek->tahun_rilis ?? '—' }}
                        </td>
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center gap-1 px-2.5 py-1 rounded-md text-xs font-medium"
                                ...
                                {{ $merek->rata_usia_optimal }} thn
                            </span>
                        </td>
                        <td class="px-5 py-4">
                            <span class="inline-flex items-center gap-1 text-sm"
                                style="color: {{ $merek->laptops_count > 0 ? 'var(--c-green)' : 'var(--c-text-muted)' }}">
                                <svg width="13" height="13" viewBox="0 0 24 24" fill="none"
                                    stroke="currentColor" stroke-width="2">
                                    <rect x="2" y="3" width="20" height="14" rx="2" />
                                    <path d="M8 21h8M12 17v4" />
                                </svg>
                                {{ $merek->laptops_count }} unit
                            </span>
                        </td>
                        ...
                        <td class="px-5 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('kepala.merek.edit', $merek->id_merek) }}" wire:navigate
                                ...
                                <button wire:click="confirmDelete({{ $merek->id_merek }})"
                                    class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-md text-xs font-medium transition-colors"
                        ...
                        <td colspan="6" class="px-5 py-16 text-center">
                            <div class="flex flex-col items-center gap-3">
                                ...
                                <p class="font-medium" style="color: var(--c-text-dim);">
                                    {{ $search ? 'Tidak ada merek yang cocok' : 'Belum ada data merek' }}
                                </p>
                                @if (!$search)
                                    <a href="{{ route('kepala.merek.create') }}" wire:navigate
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4"
            ...
            <div class="card w-full max-w-sm" style="padding: 2rem; animation: fadeInUp 0.2s ease both;">
                <div class="flex items-center gap-3 mb-4">
                    ...
                    <div>
                        <h3 class="font-semibold" style="color: var(--c-text);">Hapus Merek</h3>
                ...
                <p class="text-sm mb-6" style="color: var(--c-text-dim);">
                    Apakah Anda yakin ingin menghapus merek ini? Pastikan tidak ada laptop yang menggunakan merek ini.
                </p>
