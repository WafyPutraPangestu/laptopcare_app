<div>
    <div style="padding:2rem;max-width:800px;margin:0 auto">
            ...
            <div style="display:flex;align-items:flex-start;justify-content:space-between;gap:1rem;flex-wrap:wrap">
                <div>
                    ...
                    <p style="font-size:0.875rem;color:var(--c-text-dim);margin:0">
                        ID #{{ $jadwal->id_jadwal }} &mdash; {{ $jadwal->laptop->kode_aset }}
                    </p>
                </div>
                @php
                    $sc = match ($jadwal->status) {
                ...
                @endphp
                <span
                    style="display:inline-block;padding:0.3rem 0.875rem;border-radius:20px;font-family:var(--font-mono);font-size:0.6875rem;letter-spacing:0.04em;background:{{ $sc['bg'] }};color:{{ $sc['text'] }}">
                    {{ $jadwal->status }}
                </span>
            </div>
        </div>
        ...
        <div
            ...
            <div style="padding:1.5rem 1.75rem;border-bottom:1px solid var(--c-border)">
                ...
                <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                    ...
                    <div style="grid-column:span 2">
                        <label
                            style="display:block;font-size:0.75rem;font-weight:600;color:var(--c-text-dim);margin-bottom:0.375rem">Laptop</label>
                        <div
                            style="background:var(--c-surface-3);border:1px solid var(--c-border);border-radius:var(--radius-md);padding:0.625rem 0.875rem;font-size:0.875rem;color:var(--c-text-dim);display:flex;align-items:center;gap:0.5rem">
                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor"
                                stroke-width="2" style="color:var(--c-text-muted)">
                                <rect x="2" y="3" width="20" height="14" rx="2" ry="2" />
                                <line x1="8" y1="21" x2="16" y2="21" />
                                <line x1="12" y1="17" x2="12" y2="21" />
                            </svg>
                            {{ $jadwal->laptop->kode_aset }} — {{ $jadwal->laptop->tipe_model }}
                        </div>
                    </div>
                ...
                <div style="display:flex;flex-direction:column;gap:1rem">
                    <div>
                        <label
                            ...
                            Maintenance</label>
                        <textarea wire:model="deskripsi_maintenance" rows="3" placeholder="Jelaskan rencana pekerjaan..."
                    ...
                    {{-- Catatan teknisi & hasil (muncul jika selesai) --}}
                    @if ($status === 'Selesai')
                        <div
                            style="padding:1rem;background:var(--c-green-dim);border:1px solid rgba(45,212,160,0.2);border-radius:var(--radius-md)">
                            <div
                                style="font-family:var(--font-mono);font-size:0.6875rem;color:var(--c-green);letter-spacing:0.04em;margin-bottom:0.75rem;display:flex;align-items:center;gap:0.375rem">
                            ...
                            <div style="display:flex;flex-direction:column;gap:0.875rem">
                                <div>
                                    <label
                                        style="display:block;font-size:0.75rem;font-weight:600;color:var(--c-text-dim);margin-bottom:0.375rem">Catatan
                                        Teknisi</label>
                                    <textarea wire:model="catatan_teknisi" rows="2" placeholder="Catatan dari teknisi yang mengerjakan..."
                                        style="background:var(--c-surface-2);border:1px solid var(--c-border);border-radius:var(--radius-md);color:var(--c-text);font-size:0.875rem;padding:0.625rem 0.875rem;width:100%;resize:vertical;font-family:var(--font-body)"></textarea>
                                </div>
                                <div>
                                    <label
                                        style="display:block;font-size:0.75rem;font-weight:600;color:var(--c-text-dim);margin-bottom:0.375rem">Hasil
                                        Maintenance</label>
                                    <textarea wire:model="hasil_maintenance" rows="2" placeholder="Ringkasan hasil pekerjaan..."
                                        style="background:var(--c-surface-2);border:1px solid var(--c-border);border-radius:var(--radius-md);color:var(--c-text);font-size:0.875rem;padding:0.625rem 0.875rem;width:100%;resize:vertical;font-family:var(--font-body)"></textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:1rem">
                        ...
                        <div>
                            <label
                                style="display:block;font-size:0.75rem;font-weight:600;color:var(--c-text-dim);margin-bottom:0.375rem">Biaya
                                (Rp)</label>
            ...
            {{-- Actions --}}
            <div
                style="padding:1.25rem 1.75rem;background:var(--c-surface-2);display:flex;align-items:center;justify-content:space-between;gap:0.75rem;flex-wrap:wrap">
                <div style="font-family:var(--font-mono);font-size:0.6875rem;color:var(--c-text-muted)">
                    Dibuat: {{ $jadwal->created_at->format('d M Y, H:i') }}
                    &middot; Diperbarui: {{ $jadwal->updated_at->format('d M Y, H:i') }}
                </div>
                <div style="display:flex;gap:0.75rem">
                    ...
                    <button wire:click="update" wire:loading.attr="disabled"
                        ...
                        <span wire:loading wire:target="update" style="display:flex;align-items:center;gap:0.5rem">
                            <span
